...
int s21_from_decimal_to_float(s21_decimal src, float *dst) {
  ...
  for (int i = 0; i < 96; i++) {
    temp += s21_get_bit_on_index(src, i) * pow(2, i);
  }
  temp = temp * pow(10, -s21_get_mantissa(src));
  if (s21_get_bit_on_index(src, 127)) temp = temp * (-1);
  *dst = temp;
  return 0;
}
...
int s21_from_decimal_to_int(s21_decimal src, int *dst) {
  int error = 0;
  int scale = s21_get_mantissa(src);
  ...
  if (s21_get_bit_on_index(src, 127)) *dst = *dst * (-1);
  return error;
}
