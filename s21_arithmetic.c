...
int s21_add(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  ...
  if (s21_get_sign(value_1) != s21_get_sign(value_2)) {
    ...
    s21_set_bit_on_index(&value_1, 127, 0);
    s21_set_bit_on_index(&value_2, 127, 0);
    error = sign ? s21_sub(value_2, value_1, result)
                 : s21_sub(value_1, value_2, result);
  } else {
    s21_big_decimal v1 = {0}, v2 = {0}, r = {0};
    int scale = 0;
    s21_convert_to_big_decimal(value_1, &v1);
    s21_convert_to_big_decimal(value_2, &v2);
    int diff = s21_get_mantissa(value_1) - s21_get_mantissa(value_2);
    if (diff > 0) {
      get = s21_get_mantissa(value_1);
      s21_set_mantissa(&value_2, get);
    } else {
      get = s21_get_mantissa(value_2);
      s21_set_mantissa(&value_1, get);
    }
    s21_normalization(&v1, &v2, diff);
    s21_add_big_decimal(v1, v2, &r);
    scale = s21_post_normalization(&r, s21_get_mantissa(value_1));
    if (scale >= 0) {
      s21_convert_to_small_decimal(result, r);
      s21_set_mantissa(result, scale);
    } else {
      error = 1;
    }
  }
  ...
  if (error) s21_set_null_decimal(result);
  return error;
}
...
void s21_add_big_decimal(s21_big_decimal value_1, s21_big_decimal value_2,
  ...
  int res = 0, ovf = 0;
  for (int i = 0; i < 256; i++) {
    res = s21_get_bit_on_index_big(value_1, i) +
          s21_get_bit_on_index_big(value_2, i) + ovf;
    ovf = res / 2;
    s21_set_bit_on_index_big(result, i, res % 2);
  }
}
...
int s21_sub(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  ...
  if (s21_get_sign(value_1) != s21_get_sign(value_2)) {
    ...
    s21_convert_to_big_decimal(value_2, &v2);
    int diff = s21_get_mantissa(value_1) - s21_get_mantissa(value_2);
    diff > 0 ? s21_set_mantissa(&value_2, s21_get_mantissa(value_2) + diff)
             : s21_set_mantissa(&value_1, s21_get_mantissa(value_1) - diff);
    s21_normalization(&v1, &v2, diff);
...
void s21_sub_big_decimal(s21_big_decimal value_1, s21_big_decimal value_2,
  ...
  for (int i = 0; i < 256; i++) {
    res = s21_get_bit_on_index_big(value_1, i) -
          s21_get_bit_on_index_big(value_2, i) - tmp;
    tmp = res < 0;
    res = abs(res);
    s21_set_bit_on_index_big(result, i, res % 2);
  }
...
int s21_mul(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  ...
  scale = s21_get_mantissa(value_1) + s21_get_mantissa(value_2);
...
int s21_mul_big_decimal(s21_big_decimal value_1, s21_big_decimal value_2,
  ...
  for (int i = 0; i < 256 && !error; i++) {
    if (s21_get_bit_on_index_big(value_2, i)) {
...
int s21_div(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  ...
  if (s21_is_decimal_no_empty(value_2)) {
    ...
    s21_set_mantissa(&value_1, s21_get_mantissa(value_1) + scale);
    res_scale = s21_get_mantissa(value_1) - s21_get_mantissa(value_2);
    if (res_scale > 0) {
      res_scale = s21_post_normalization(&r, res_scale);
    } else if (res_scale < 0) {
      s21_increase_mantissa_big_decimal(&r, abs(res_scale));
      res_scale = s21_post_normalization(&r, 0);
    }
    if (res_scale >= 0) {
      s21_convert_to_small_decimal(result, r);
      s21_set_mantissa(result, res_scale);
    } else {
      error = 1;
    }
  } else {
...
int s21_div_big_decimal(s21_big_decimal value_1, s21_big_decimal value_2,
  ...
  for (int i = 0; i < 96 && s21_is_big_decimal_not_empty(value_1);) {
    if (i > 0) {
      ...
      s21_increase_mantissa_big_decimal(result, 1);
      s21_increase_mantissa_big_decimal(&value_1, 1);
      save_scale++;
    }
    ...
    for (; diff >= 0 && s21_is_big_decimal_not_empty(value_1);) {
      if (s21_is_greater_big_decimal(value_2, value_1)) {
        s21_set_bit_on_index_big(&tmp, 0, 0);
      } else {
        s21_sub_big_decimal(value_1, value_2, &value_1);
        s21_set_bit_on_index_big(&tmp, 0, 1);
      }
      i++;
      diff--;
...
int s21_mod(s21_decimal value_1, s21_decimal value_2, s21_decimal *result) {
  ...
  s21_set_bit_on_index(result, 127, sign);
  if (error == 1 && s21_get_sign(*result)) error = 2;
  if (error) s21_set_null_decimal(result);
  return error;
}
...
int s21_get_integer_part(s21_decimal value_1, s21_decimal value_2,
  ...
  while (s21_is_greater_big_decimal(v2, v1)) {
    s21_increase_mantissa_big_decimal(&v1, 1);
    s21_set_mantissa(&value_1, s21_get_mantissa(value_1) + 1);
  }
  int scale = s21_get_mantissa(value_2) - s21_get_mantissa(value_1);
  s21_find_highest_bit_big_decimal(v1, v2, &b1, &b2);
  for (int j = b1; j >= 0; j--) {
    s21_set_bit_on_index_big(&nvalue, j, !s21_get_bit_on_index_big(v2, j));
  }
  s21_add_big_decimal(nvalue, one, &nvalue);
  for (int j = b1; j >= 0; j--) {
    s21_shift_big_dec_left(&tmp_rem, 1);
    high_bit = s21_get_bit_on_index_big(tmp_rem, b1);
    s21_set_bit_on_index_big(&tmp_rem, b1 + 1, 0);
    s21_set_bit_on_index_big(&tmp_rem, 0, s21_get_bit_on_index_big(v1, b1));
    s21_shift_big_dec_left(&v1, 1);
    s21_set_bit_on_index_big(&v1, b1 + 1, 0);
    if (high_bit) {
    ...
    s21_set_bit_on_index_big(&tmp_rem, b1 + 1, 0);
    high_bit = s21_get_bit_on_index_big(tmp_rem, b1);
    s21_set_bit_on_index_big(&v1, 0, !high_bit);
  }
  if (scale < 0) {
    s21_increase_mantissa_big_decimal(&v1, -scale);
  ...
  if (scale >= 0) {
    s21_set_mantissa(result, scale);
    s21_convert_to_small_decimal(result, v1);
  } else {
    error = 1;
  }
  if (error == 1 && s21_get_sign(*result)) error = 2;
...
void s21_set_bit_on_index_big(s21_big_decimal *dst, int index, int bit) {
...
int s21_get_bit_on_index_big(s21_big_decimal dst, int index) {
  int mask = 1u << (index % 32);
...
void s21_shift_big_dec_right(s21_big_decimal *dst, int num) {
  ...
  for (int k = 0; k < num; k++) {
    for (int i = 0; i < 7; i++) {
      buffer[i] = s21_get_bit_on_index_big(*dst, (i + 1) * 32);
    }
    for (int i = 0; i < 7; i++) {
      dst->bits[i] >>= 1;
      s21_set_bit_on_index_big(dst, (i + 1) * 32 - 1, buffer[i]);
    }
    dst->bits[7] >>= 1;
  }
}
...
int s21_shift_big_dec_left(s21_big_decimal *dst, int num) {
  ...
  for (int k = 0; k < num; k++) {
    for (int i = 0; i < 7; i++) {
      buffer[i] = s21_get_bit_on_index_big(*dst, (i + 1) * 32 - 1);
    }
    for (int i = 7; i > 0 && !error; i--) {
      if (s21_get_bit_on_index_big(*dst, 255)) error = 1;
      dst->bits[i] <<= 1;
      s21_set_bit_on_index_big(dst, i * 32, buffer[i - 1]);
    }
    dst->bits[0] <<= 1;
  }
  return error;
...
void s21_normalization(s21_big_decimal *value_1, s21_big_decimal *value_2,
                       int diff) {
  if (diff > 0) {
    s21_increase_mantissa_big_decimal(value_2, diff);
  } else if (diff < 0) {
    s21_increase_mantissa_big_decimal(value_1, -diff);
  }
}
...
int s21_post_normalization(s21_big_decimal *result, int scale) {
  ...
  while ((result->bits[3] || result->bits[4] || result->bits[5] ||
    ...
    s21_decrease_mantissa_big_decimal(result, 1);
    scale--;
  }
  if (dop && scale == 0 && result->bits[3] == 0 &&
      s21_get_bit_on_index_big(*result, 0))
    s21_set_bit_on_index_big(result, 0, 0);
...
void s21_increase_scale_decimal(s21_decimal *dst, int n) {
  ...
  int scale = s21_get_mantissa(*dst);
  for (int i = 0; i < n; i++) {
    s21_mul(*dst, ten, &tmp);
    *dst = tmp;
    s21_set_null_decimal(&tmp);
  }
  s21_set_mantissa(dst, scale + n);
}

void s21_increase_mantissa_big_decimal(s21_big_decimal *dst, int n) {
...
void s21_decrease_mantissa_big_decimal(s21_big_decimal *dst, int n) {
  s21_big_decimal ten = {{10, 0, 0, 0, 0, 0, 0, 0}}, tmp = {0};
  for (int i = 0; i < n; i++) {
...
void s21_find_highest_bit_decimal(s21_decimal v1, s21_decimal v2, int *bit_1,
  ...
  for (int i = 95; i >= 0 && (!(*bit_1) || !(*bit_2)); i--) {
    if (*bit_1 == 0 && s21_get_bit_on_index(v1, i)) *bit_1 = i;
    if (*bit_2 == 0 && s21_get_bit_on_index(v2, i)) *bit_2 = i;
  }
}
...
void s21_find_highest_bit_big_decimal(s21_big_decimal v1, s21_big_decimal v2,
  ...
  for (int i = 255; i >= 0 && (!(*bit_1) || !(*bit_2)); i--) {
    if (*bit_1 == 0 && s21_get_bit_on_index_big(v1, i)) *bit_1 = i;
    if (*bit_2 == 0 && s21_get_bit_on_index_big(v2, i)) *bit_2 = i;
  }
}
